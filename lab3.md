...
This command will be very useful if we only want to know what the person in emergency said. The command will exclude any line containing "?", which very likely contains questions from the person at the 911 Call Center. Therefore, with this command, we can exclude questions asked by the 911 center and examine only the things the caller said.

This might be useful in cases when we want to investigate into the caller's behavior. For example, if it turns out that the caller is the actual suspect for the incident they called 911 for, this command might help.
...
`grep -R Isotopic ./technical/biomed/`
